            ...
            >
                <ScrollView contentContainerStyle={styles.scrollContent} keyboardShouldPersistTaps="handled">
                        ...
                        <Text style={[styles.title, { color: theme.text }]}>
                            {isForgotPassword ? (
                                forgotPasswordStep === 'email' ? 'Forgot Password?' :
                                    forgotPasswordStep === 'otp' ? 'Verify Code' : 'Set New Password'
                            ) : (
                                isSignUp ? 'Sali Ka Na!' : 'Welcome Back Perds!'
                            )}
                        </Text>
                        <Text style={[styles.subtitle, { color: theme.muted }]}>
                            {isForgotPassword ? (
                                forgotPasswordStep === 'email' ? 'Lagay mo email mo para padalhan ka namin ng recovery code.' :
                                    forgotPasswordStep === 'otp' ? 'I-type mo yung 6-digit code na pinadala namin sa email mo.' : 'Gawa ka ng bagong password na hindi mo na makakalimutan.'
                            ) : (
                                isSignUp ? 'Gawa ka ng account para ma-track ang expenses mo.' : 'Mag-Login para ma-access ang iyong financial insights.'
                            )}
                        </Text>
                    </View>
                    ...
                    <View style={styles.form}>
                        ...
                        {/* Email Input */}
                        {((!isSignUp && !isForgotPassword) || (isSignUp && signUpStep === 'email') || (isForgotPassword && forgotPasswordStep === 'email')) && (
                            <View style={[styles.inputWrapper, { backgroundColor: theme.card, borderColor: theme.cardBorder }]}>
                        ...
                        {/* OTP Input */}
                        {((isSignUp && signUpStep === 'otp') || (isForgotPassword && forgotPasswordStep === 'otp')) && (
                            <View style={[styles.inputWrapper, { backgroundColor: theme.card, borderColor: theme.cardBorder }]}>
                        ...
                        {/* Password Input */}
                        {((!isSignUp && !isForgotPassword) || (isSignUp && signUpStep === 'password') || (isForgotPassword && forgotPasswordStep === 'password')) && (
                            <View style={[styles.inputWrapper, { backgroundColor: theme.card, borderColor: theme.cardBorder }]}>
                                {renderIcon(Lock, 20, theme.muted)}
                                <TextInput
                                    style={[styles.input, { color: theme.text, outline: 'none' } as any]}
                        ...
                        {!isSignUp && !isForgotPassword && (
                            <TouchableOpacity
                                ...
                                activeOpacity={0.6}
                            >
                                <Text style={{ color: theme.green, textAlign: 'right', fontSize: 13, fontWeight: '600', textDecorationLine: 'underline' }}>
                                    Forgot Password?
                                </Text>
                            </TouchableOpacity>
                        )}
                        ...
                        <TouchableOpacity
                            ...
                            onPress={() => {
                                if (isForgotPassword) {
                                    ...
                                    else if (forgotPasswordStep === 'password') completeSignUpWithPassword();
                                } else {
                                    if (!isSignUp) signInWithEmail();
                                    else if (signUpStep === 'email') sendOtp();
                                    else if (signUpStep === 'otp') verifyOtpCode();
                                    else if (signUpStep === 'password') completeSignUpWithPassword();
                                }
                            }}
                            disabled={loading}
                        >
                                ...
                                <Text style={styles.mainButtonText}>
                                    {isForgotPassword ? (
                                        forgotPasswordStep === 'email' ? 'Send Reset Code' :
                                            forgotPasswordStep === 'otp' ? 'Verify Code' : 'Update Password'
                                    ) : (
                                        !isSignUp ? 'Login' :
                                            signUpStep === 'email' ? 'Send OTP' :
                                                signUpStep === 'otp' ? 'Verify OTP' : 'Complete Setup'
                                    )}
                                </Text>
                            )}
                        </TouchableOpacity>
                        ...
                        >
                            <Text style={{ color: theme.muted }}>
                                {isForgotPassword ? 'Balik sa ' : (isSignUp ? 'May account na? ' : 'Wala pang account? ')}
                                <Text style={{ color: theme.green, fontWeight: '700' }}>
                                    {isForgotPassword ? 'Login perds' : (isSignUp ? 'Login dito' : 'Signup dito')}
                                </Text>
                            </Text>
                        </TouchableOpacity>
                    </View>
