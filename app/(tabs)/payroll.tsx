        ...
        <View style={[styles.card, { backgroundColor: theme.card, borderColor: theme.cardBorder }]}>
            ...
            <View style={styles.cardInfo}>
                ...
                <View style={styles.weeksRow}>
                    <Text style={[styles.weekLabel, { color: theme.muted }]}>W1: <Text style={{ color: theme.text }}>₱{item.week1.toLocaleString()}</Text></Text>
                    <Text style={[styles.weekLabel, { color: theme.muted }]}>W2: <Text style={{ color: theme.text }}>₱{item.week2.toLocaleString()}</Text></Text>
                </View>
            </View>
            <View style={styles.cardRight}>
                <Text style={[styles.totalAmount, { color: theme.green }]}>₱{(item.week1 + item.week2).toLocaleString()}</Text>
        ...
        <SafeAreaView style={[styles.container, { backgroundColor: theme.background }]} edges={['top']}>
            <View style={styles.header}>
                <View>
                    ...
                    <Text style={[styles.subtitle, { color: theme.muted }]}>Total: ₱{totalPayroll.toLocaleString()}</Text>
                </View>
                <TouchableOpacity
                ...
                <FlatList
                    ...
                    ListEmptyComponent={
                        <View style={styles.emptyState}>
                            <Text style={{ color: theme.muted }}>Wala ka pang empleyado perds.</Text>
                        </View>
                    }
                />
            )}
                ...
                <View style={styles.modalOverlay}>
                    <View style={[styles.modalContent, { backgroundColor: theme.card, borderColor: theme.cardBorder }]}>
                        <View style={styles.modalHeader}>
                            <Text style={[styles.modalTitle, { color: theme.text }]}>Add Employee Pay</Text>
                        ...
                        <View style={styles.row}>
                            <View style={[styles.inputGroup, { flex: 1 }]}>
                                <Text style={[styles.label, { color: theme.muted }]}>Week 1</Text>
                                <TextInput
                                    style={[styles.input, { color: theme.text, backgroundColor: theme.background, borderColor: theme.cardBorder }]}
                                    value={week1}
                            ...
                            <View style={[styles.inputGroup, { flex: 1 }]}>
                                <Text style={[styles.label, { color: theme.muted }]}>Week 2</Text>
                                <TextInput
                                    style={[styles.input, { color: theme.text, backgroundColor: theme.background, borderColor: theme.cardBorder }]}
                                    value={week2}
                        ...
                        <TouchableOpacity style={[styles.saveButton, { backgroundColor: theme.green }]} onPress={handleAddEntry}>
...
const styles = StyleSheet.create({
    ...
    saveButton: { height: 56, borderRadius: 16, justifyContent: 'center', alignItems: 'center', marginTop: 8 },
